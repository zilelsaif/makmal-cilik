...
(() => {
  ...
  function render(name, context) {
    ...
    if (name === 'experiment' || name === 'year3Experiment' || name === 'year4Experiment' || name === 'year5Experiment' || name === 'year6Experiment') {
      screen.innerHTML = `<div class="content-screen experiment-screen">${nav()}<div id="experiment-root"></div></div>`;
      disposeExperiment = window.MakmalExperiment.mount(document.getElementById('experiment-root'), { missionId: context.missionId, onExit: () => router.navigate(name==='year6Experiment'?'year6Unit':name==='year5Experiment'?'year5Unit':name==='year4Experiment'?'year4Unit':name==='year3Experiment'?'year3Unit':'unitDetail', { unitId: context.unitId }) });
      return;
    }
    ...
    if (name === 'yearSelect') content = `${heading('PILIH TAHUN', 'Mulakan perjalanan Sains kamu.', 'MULA BELAJAR')}<div class="year-grid">${[1,2,3,4,5,6].map(year => `<button class="year-card ${[2,3,4,5,6].includes(year) ? 'active' : ''}" ${[2,3,4,5,6].includes(year) ? `data-route="year${year}"` : 'data-message="Kandungan Tahun ini akan datang."'}><span class="year-number" aria-hidden="true">0${year}</span><span class="year-arrow" aria-hidden="true">${[2,3,4,5,6].includes(year) ? '↗' : '🔒'}</span><strong>TAHUN ${year}</strong><span class="badge">${[2,3,4,5,6].includes(year) ? 'Aktif' : 'Akan Datang'}</span></button>`).join('')}</div>`;
