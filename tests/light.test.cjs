...
for(const value of [null,'{bad',JSON.stringify({version:'0.5.0',profile:{name:'Aina'},settings:{sound:false},extra:3,progress:{year2:{electricity,plants:{keep:1},lightDark:{mission3:{completed:true,attempts:4,completedAt:'first'}}}}})]){
 raw=value;p.loadData();assert.equal(p.getData().version,'2.4.0');const before=JSON.stringify([p.getData().progress.year2.electricity,p.getData().settings,p.getData().profile,p.getData().progress.year2.plants,p.getData().extra]);assert(p.isAvailable(1,'lightDark'));
