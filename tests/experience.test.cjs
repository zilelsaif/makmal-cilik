...
const saved={version:'0.7.0',settings:{sound:false,extra:8},profile:{name:'Aina'},progress:{year2:{electricity:all,lightDark:all,mixtures:all,plants:{keep:1}}},extra:3};raw=JSON.stringify(saved);p.loadData();assert.equal(p.getData().version,'2.0.0');p.saveData();const migrated=JSON.parse(raw);delete migrated.version;delete saved.version;assert.deepEqual(migrated.progress.year1,{});delete migrated.progress.year1;assert.deepEqual(migrated.progress.year3,{});assert.deepEqual(migrated.progress.year4,{});assert.deepEqual(migrated.progress.year5,{});assert.deepEqual(migrated.progress.year6,{});delete migrated.progress.year3;delete migrated.progress.year4;delete migrated.progress.year5;delete migrated.progress.year6;assert.deepEqual(migrated,saved);
