...
document.getElementById('run').onclick=async()=>{
 ...
 const seed=mixtures=>JSON.stringify({version:'0.6.0',settings:{sound:false},profile:{name:'Aina'},extra:7,progress:{year2:{electricity:all,lightDark:all,plants:{keep:9},mixtures}}});
 const scenarios=[['fresh',null],['v0.6',seed({})],['partial',seed({mission3:record})],['complete',seed(all)],['malformed','{bad'],['blocked',null]];
 try{
  ...
  for(const [width,height] of [[1366,768],[1920,1080],[390,844],[360,640],[800,450]])for(const [name,raw] of scenarios){
   ...
   const w=frame.contentWindow,d=frame.contentDocument,p=w.MakmalProgress;let completions=0;const complete=p.completeMission;p.completeMission=(...args)=>{completions++;return complete(...args);};
