...
const saved={version:'0.7.0',settings:{sound:false,extra:8},profile:{name:'Aina'},progress:{year2:{electricity:all,lightDark:all,mixtures:all,plants:{keep:1}}},extra:3};raw=JSON.stringify(saved);p.loadData();assert.equal(p.getData().version,'2.5.1');p.saveData();const migrated=JSON.parse(raw),player=migrated.profiles[0];assert.equal(migrated.version,'2.5.1');assert.equal(migrated.settings.sound,false);assert.equal(migrated.settings.extra,8);assert.equal(migrated.extra,3);assert.equal(migrated.activeProfileId,player.id);assert.equal(player.name,'Aina');assert.deepEqual(player.progress.year1,{});assert.deepEqual(player.progress.year3,{});assert.deepEqual(player.progress.year4,{});assert.deepEqual(player.progress.year5,{});assert.deepEqual(player.progress.year6,{});assert.deepEqual(player.progress.year2,saved.progress.year2);assert(!('progress' in migrated)&&!('profile' in migrated));
...
console.log('PASS: 15 objectives/dialogue sets, 60 phase hint paths, contextual switch/stir hints, compact completion, no repeated unit celebration on ordinary replay, exact v0.7 profile migration.');
