document.getElementById('run').onclick=async()=>{
 ...
 assert(p.year2CompletedCount()===35,'Progress lost');press();d.querySelector('[data-exit]').click();await Promise.resolve();assert(w.qaExit===1,'Explicit exit failed');assert(!w.qaErrors.length,'Errors');out.textContent+=`PASS ${width}×${height}: 35 mission Back routes, rotation/resume retained state, safe areas, finale, exit confirmation, no errors.\n`;
