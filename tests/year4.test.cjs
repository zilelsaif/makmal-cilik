...
for(const scenario of ['fresh','v1.2','partial','unit','full','malformed','blocked','bad-records']){
 ...
 p.loadData();assert.equal(p.APP_VERSION,'2.5.1');const before=JSON.stringify([p.getData().progress.year2,p.getData().progress.year3,p.getData().settings,p.getData().profile,p.getData().extra]);
