...
for(const value of [null,'{bad',JSON.stringify({version:'0.6.0',settings:{sound:false},profile:{name:'Aina'},progress:{year2:{electricity:all,lightDark:all,plants:{keep:true},mixtures:{mission3:old}}}})]){
 raw=value;p.loadData();assert.equal(p.getData().version,'2.0.0');const before=JSON.stringify([p.getData().settings,p.getData().profile,p.getData().progress.year2.electricity,p.getData().progress.year2.lightDark,p.getData().progress.year2.plants]);
