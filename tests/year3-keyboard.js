...
(()=>{let frame,w,d,session,defs,index=0,keys=0;const guide=document.getElementById('guide'),results=document.getElementById('results'),advance=document.getElementById('advance');
 ...
 function open(){w.MakmalRouter.navigate('year3Experiment',{unitId:defs[index].unitId,missionId:defs[index].id});update();}
 function update(){if(!session)return;let selector;
  ...
  if(session.state.step===1){const [a,v]=w.MakmalYear3Domains[defs[index].domain].hint(session.state,defs[index]);selector=`[data-exp="${a}"]${v===undefined?'':`[data-value="${v}"]`}`;}
