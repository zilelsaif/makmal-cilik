...
for (const name of ['index.html', 'css', 'js', 'assets']) if (!fs.existsSync(path.join(root, name))) throw Error('Missing source: ' + name);
...
function copy(source, target) {
  ...
  } else if (/\.(html|css|js|json|svg|webp|png|jpg|jpeg|gif|woff2?|ttf|mp3|ogg|wav|m4a)$/i.test(source)) {
    fs.copyFileSync(source, target); count++; bytes += stat.size;
  }
}
for (const name of ['index.html', 'css', 'js', 'assets']) copy(path.join(root, name), path.join(output, name));
