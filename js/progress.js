...
window.MakmalProgress = (() => {
  const APP_VERSION='2.1.0', KEY='makmalCilikData', MAX_PROFILES=6;
  ...
  function resetProfile(id){const p=current.profiles.find(x=>x.id===id);if(!p)return false;p.progress=emptyProgress();p.lastPlayedAt=new Date().toISOString();if(p.id===current.activeProfileId)attachAliases(current);persist();return true;}
  ...
  return {APP_VERSION,MAX_PROFILES,AVATARS,cleanName,loadData,saveData,updateSetting,getData,getProfiles,getActiveProfile,switchProfile,addProfile,updateProfile,deleteProfile,resetProfile,profileStats,summaryForProgress,forYear,year1Units,year3Units,year4Units,year5Units,year6Units,allYearsSummary,allYearsCompletedTotal,allYearsCompletedUnits,allYearsComplete,storageUnit,year2StorageUnits,year2CompletedCount,year2CompletedUnits,isYear2Complete,isMissionComplete,completedCount,isUnitComplete,isAvailable,startMissionAttempt,completeMission};
})();
