...
(() => {
  ...
  function render(name, context) {
    ...
    if (name === 'yearSelect') {const years=progress.allYearsSummary();content = `${heading('PILIH TAHUN', 'Mulakan perjalanan Sains kamu.', 'MULA BELAJAR')}<div class="year-grid">${years.map(item => `<button class="year-card active ${item.completed===item.total?'year-complete':''}" data-route="year${item.year}" aria-label="Tahun ${item.year}, ${item.completed} daripada ${item.total} eksperimen${item.completed===item.total?', selesai':''}"><span class="year-number" aria-hidden="true">0${item.year}</span><span class="year-arrow" aria-hidden="true">↗</span><strong>TAHUN ${item.year}</strong><span class="year-progress-copy">${item.completed} / ${item.total} eksperimen · ${item.units} / ${item.unitTotal} unit</span><span class="badge">${item.completed===item.total?'✓ Selesai':item.completed?'Sedang Diterokai':'Sedia Dimulakan'}</span></button>`).join('')}</div>${window.MakmalMaster.progressView()}`;}
