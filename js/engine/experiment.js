...
window.MakmalExperiment = (() => {
  ...
  function mount(root, { onExit, missionId = 'electricity-1' }) {
    ...
    const circuit = !year3 && !discovery && !plants && !mixture && !light && missionId !== 'electricity-1';
    const definition = year3 ? window.MakmalYear3.missions[missionId] : discovery ? window.MakmalNewContent.missions[missionId] : plants ? window.MakmalPlantContent.missions[missionId] : mixture ? window.MakmalMixtureContent.missions[missionId] : light ? window.MakmalLightContent.missions[missionId] : circuit ? window.MakmalCircuitMissions[missionId] : window.MakmalMission1;
    const progressKey = definition.progressKey || 'mission1';
    const storageUnit = definition.storageUnit || 'electricity';
    const session = year3 ? window.MakmalYear3Lab.createSession(definition) : discovery ? window.MakmalDiscovery.createSession(definition) : plants ? window.MakmalPlants.createSession(definition) : mixture ? window.MakmalMixture.createSession(definition) : light ? window.MakmalLight.createSession(definition) : circuit ? window.MakmalCircuit.createSession(definition) : createSession(definition);
    const progress = year3 ? window.MakmalProgress.forYear(3) : window.MakmalProgress;
    const yearComplete=()=>year3?progress.isYearComplete():progress.isYear2Complete();
    ...
    function draw(focusSelector) {
      ...
      let activity = '';
      if (!year3 && !discovery && !plants && !circuit && !light && !mixture) {
    ...
    function dispatch(action, value, target) {
      ...
      else if((year3||discovery||plants||light||mixture)&&s.effect){event=s.effect;feedback=event==='wrong'?'wrong':['correct','itemFound','magnetPickup','sieveAction','stirring','watering','plantGrowth','correctMatch','recovery','measurement','classification','observation'].includes(event)?'correct':'';}
    ...
    root.addEventListener('click', event => {
      ...
      if (action === 'yearComplete') { window.MakmalRouter.navigate(year3?'year3Complete':'year2Complete'); window.MakmalRewards.play('click'); return; }
      if (action === 'year2') { window.MakmalRouter.navigate(year3?'year3':'year2'); window.MakmalRewards.play('click'); return; }
