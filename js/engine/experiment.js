...
window.MakmalExperiment = (() => {
  ...
  function mount(root, { onExit, missionId = 'electricity-1' }) {
    ...
    const year6=Boolean(window.MakmalYear6?.missions[missionId]);
    ...
    function draw(focusSelector) {
      ...
      let activity = '';
      if (!year3 && !year4 && !year5 && !year6 && !discovery && !plants && !circuit && !light && !mixture) {
      ...
      if (year6 && s.step < 4) activity = window.MakmalYear6Lab.render(s, definition);
    ...
    function dispatch(action, value, target) {
      ...
      else if((year6||year5||year4||year3||discovery||plants||light||mixture)&&s.effect){event=s.effect;feedback=event==='wrong'?'wrong':['correct','itemFound','magnetPickup','sieveAction','stirring','watering','plantGrowth','correctMatch','recovery','measurement','classification','observation'].includes(event)?'correct':'';}
    ...
    root.addEventListener('click', event => {
      ...
      if (action === 'yearComplete') { window.MakmalRouter.navigate(year6?'year6Complete':year5?'year5Complete':year4?'year4Complete':year3?'year3Complete':'year2Complete'); window.MakmalRewards.play('click'); return; }
      if (action === 'year2') { window.MakmalRouter.navigate(year6?'year6':year5?'year5':year4?'year4':year3?'year3':'year2'); window.MakmalRewards.play('click'); return; }
