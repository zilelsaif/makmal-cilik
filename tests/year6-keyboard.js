...
(()=>{let frame,w,d,session,defs,index=0,keys=0;const guide=document.getElementById('guide'),results=document.getElementById('results'),advance=document.getElementById('advance');
 const next=()=>{index++;advance.hidden=true;if(index>=defs.length){guide.textContent='ALL 55 KEYBOARD MISSIONS PASSED';return;}open();};
 function open(){w.MakmalRouter.navigate('year6Experiment',{unitId:defs[index].unitId,missionId:defs[index].id});update();}
 function update(){if(!session)return;let selector;
  ...
  if(session.state.step===1){const [a,v]=w.MakmalYear6Domains[defs[index].mode].hint(session.state,defs[index]);selector=`[data-exp="${a}"]${v===undefined?'':`[data-value="${v}"]`}`;}
