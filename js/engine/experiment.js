...
window.MakmalExperiment = (() => {
  ...
  function mount(root, { onExit, missionId = 'electricity-1' }) {
    const year1=Boolean(window.MakmalYear1?.missions[missionId]);
    ...
    function draw(focusSelector) {
      ...
      let activity = '';
      if (!year1 && !year3 && !year4 && !year5 && !year6 && !discovery && !plants && !circuit && !light && !mixture) {
      ...
      if (year1 && s.step < 4) activity = window.MakmalYear1Lab.render(s, definition);
    ...
    function dispatch(action, value, target) {
      ...
      else if((year1||year6||year5||year4||year3||discovery||plants||light||mixture)&&s.effect){event=s.effect;feedback=event==='wrong'?'wrong':['correct','itemFound','magnetPickup','sieveAction','stirring','watering','plantGrowth','correctMatch','recovery','measurement','classification','observation'].includes(event)?'correct':'';}
