...
document.getElementById('run').addEventListener('click', async () => {
  ...
  try {
    ...
    for (const [name, raw] of scenarios) {
      ...
      const click = selector => { const e=d.querySelector(selector);assert(e&&!e.disabled,'Unavailable '+selector);e.click(); };
      ...
      click('[data-exp="exit"]');assert(d.querySelector('[data-mission="electricity-1"]').textContent.includes('Selesai'),'List stale');assert(d.querySelector('[data-mission="electricity-2"]').textContent.includes('Akan Datang'),'Mission2 unlocked');
      ...
      output.textContent += `PASS ${name}: 3 completions, replay retention, attempted-run counts, step gates, hints, cancellation, asset fallback, navigation, no runtime errors.\n`;
      await delay();
    }
